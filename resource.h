...
// Variáveis da Memória Compartilhada
...
NTSTATUS create_shared_memory()
{
	...
	//-----------------------------------------------------------------------------	
	//	 ZwMapViewOfSection
	//	-lMaxSize é a quantidade de 'Espaço' que o MapViewOfSection vai olhar
	//	-ViewSize é o quanto do 'Espaço' será mapeado (se 0, começa do início)
	//-----------------------------------------------------------------------------	
	...
	if (!NT_SUCCESS(Status))
	{
		...
		ZwClose(g_Section); //Fecha Handle
		return Status;
	}
...
NTSTATUS create_shared_memory_esp()
{
	...
	if (!NT_SUCCESS(Status))
	{
		...
		ZwClose(g_Section2); //Fecha Handle
		return Status;
	}
...
here:; // precisa de loop porque processos recém-criados tornam o buffer pequeno demais
	if (!NT_SUCCESS(ZwQuerySystemInformation(SystemProcessInformation, NULL, NULL, &CallBackLength)))
	{
		...
		do
		{
			...
			PSI = (PSYSTEM_PROCESS_INFO)((unsigned char*)PSI + PSI->NextEntryOffset); // Calcula o endereço da próxima entrada.
...
BOOLEAN write_to_read_only_memory(void* address, void* buffer, size_t size) {
	...
	// Bloqueando e mapeando memória com direitos de leitura e escrita:
	MmProbeAndLockPages(Mdl, KernelMode, IoReadAccess);
	PVOID Mapping = MmMapLockedPagesSpecifyCache(Mdl, KernelMode, MmNonCached, NULL, FALSE, NormalPagePriority);
	MmProtectMdlSystemAddress(Mdl, PAGE_READWRITE);

	// Escrevendo o buffer no mapeamento:
...
BOOLEAN call_kernel_function(const char* module_name, void* kernel_function_address, LPCSTR funct_name) {
	...
	//	0xFF, 0xE0, // jmp rax // pula de volta
	//	0x90, 0x90
	//}; // 28 bytes + 2 nops = 30 bytes	
	...
	BYTE shell_code[] = {
		0x48, 0xB8, // mov rax, 
		0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, // [xxx]
		0xFF, 0xE0, // jmp rax // d0 para chamada
	};
...
BOOLEAN call_kernel_function_ts(void* kernel_function_address) {
	...
	/*
	basta sobrescrever os primeiros 15 bytes de qualquer função no dxgkrnl.sys
	*/
